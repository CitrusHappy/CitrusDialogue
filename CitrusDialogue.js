...
function dialog(e)
{
    ...
    entireDialog = entireDialog.replace(new RegExp("{player}", "i"), _PLAYER.getDisplayName())
    ...
    */
    ...
    if(e instanceof Java.type('noppes.npcs.api.event.DialogEvent'))
    {
        runDelay(0, function(){_PLAYER.showCustomGui(_GUI);});
    }
}
...
function createDialogUpdateTimers(splitDialogString)
{
    ...
    if(splitDialogString.length != 0)
    {
        ...
        for(var i = 0; i < splitDialogString.length + 1; i++)
        {
            ...
            runDelay(pauseTime*delay + 0.05 + i*0.05/speed, function(){
    ...
    for (var i = 0; i < options.length; i++)
    {
        ...
        switch(dialogType)
        {
            ...
            case 1:
                ...
                potentialNextDialog[buttonId] = _DIALOG.getOption(i).getDialog();
                break;
            case 0:
                //QUIT_OPTION
        ...
        _GUI.addButton(DIALOG_OPTIONS_BUTTON_ID+i, _DIALOG.getOption(i).getName(), 20, 215 + 25*i)
